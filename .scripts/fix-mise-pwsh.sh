...
# Map to PowerShell release naming
case "$OS" in
    linux)
        case "$ARCH" in
            x86_64) ASSET="powershell-*-linux-x64.tar.gz" ;;
            aarch64) ASSET="powershell-*-linux-arm64.tar.gz" ;;
            *) echo "Unsupported architecture: $ARCH"; exit 1 ;;
        esac
        ;;
    darwin)
        case "$ARCH" in
            x86_64) ASSET="powershell-*-osx-x64.tar.gz" ;;
            arm64) ASSET="powershell-*-osx-arm64.tar.gz" ;;
            *) echo "Unsupported architecture: $ARCH"; exit 1 ;;
        esac
        ;;
    *)
...
# Download URL
DOWNLOAD_URL="https://github.com/PowerShell/PowerShell/releases/download/v${VERSION}/powershell-${VERSION}-${OS}-${ARCH}.tar.gz"
